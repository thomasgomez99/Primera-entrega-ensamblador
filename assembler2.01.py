...
# -*- coding: utf-8 -*-
"""
RV32I Two-Pass Assembler — v2 (clean & complete for base ISA)
=============================================================
...
- Directivas: .text (comienza en 0x00000000) y .data (registra labels, no emite bytes en esta versión).
- Salida: un word de 32 bits por línea en HEX y BIN (agrupado por campos).
- Errores: sintaxis, instrucción inválida, operandos, label indefinida, rango de inmediatos.
...
# Permite aceptar nombres legibles = temporales t0..t6 - guardados s0..s11 - argumentos a0..a7
# Se usa por el parser para convertir operandos de texto a índices de registro.
# -----------------------------------------------------------------------------
...
def format_bin_grouped(word: int) -> str:
    ...
    # fallback (no debería ocurrir en RV32I base)
...
class Assembler:
    def __init__(self, lines: List[str]):
        ...
        self.addr_data = 0x10000000  # base simbólica para .data (no se emite en v2)
    ...
    def pass1(self):
        ...
        for idx, raw in enumerate(self.lines, 1):
            ...
            if line.startswith('.'):
                ...
                else: pass  # otras directivas ignoradas
                continue
